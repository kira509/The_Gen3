...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    e.preventDefault()
    try {
      const res = await axios.post("https://movieflix-backend-ikbt.onrender.com/api/auth/login", {
        email, password
      })
    ...
    <div className="flex justify-center items-center h-screen">
      <form onSubmit={handleLogin} className="bg-zinc-900 p-8 rounded shadow w-80">
        ...
        <input type="email" placeholder="Email" value={email}
          ...
          className="w-full mb-3 p-2 rounded bg-zinc-800 border border-zinc-700 text-white" required />
        <input type="password" placeholder="Password" value={password}
          onChange={(e) => setPassword(e.target.value)}
          className="w-full mb-4 p-2 rounded bg-zinc-800 border border-zinc-700 text-white" required />
        <button type="submit"
